...
note:
- gurezo/chirimen-lite-consoleでの、改善提案/実装
    - 改善内容
        - UI の改善
            - ピンアサインの画像をサイドパネルなどですぐ確認できると良い
        - ブラウザ判定
            - Web Serial 非対応ブラウザでの警告
        - Web Serial 通信を Promise ベース直書きから既製ライブラリ(Angular/RxJs)経由で洗練
    - 方針
        - 個人的に作っているもので、要望があれば移管しても良い
        - 移管しても良いが課題がある状態（２０～３０％の機能）
        - デバイス登録や管理の仕組みができないと外部有志サイト状態のままか⇒これはchirimen-lite-consoleの話ではない
        - メンテしやすい状態にはしたい
            - angular, RxJs利用のこのコミュニティでのパス係数は？
- satakagi/PiZeroWebSerialConsole の内容
    - 改善内容
        - ログに残っていないがgurezo/chirimen-lite-consoleの提案の趣旨を推測し、chirimen-oh/PiZeroWebSerialConsoleをfork/リファクタリング/改善
        - モノリス⇒モジュール
        - グローバル変数による制御をESMで隠ぺい
        - RxJsで得られる特性のうち、PiZeroWebSerialConsoleで有用な部分をESM/Promiseで実装
            - 外部フレームワークは不使用(Vanilla)
        - レスポンシブ化、UIのルックの改善
        - PiZeroWebSerialConsole既存機能を全て継承
    
    - 方針
        - chirimen-oh/PiZeroWebSerialConsole にプルリクエストして合流させる
...
- https://chirimen-device-dashboard.web.app/ は、デバイス軸のページと位置づけ、ユースケース軸とは分けて考える
...
- CHIRIMEN コミュニティとしては、次回のエントリーはオンラインのみ申し込む
    - 個人サークルで申し込みをする為、CHIRIMEN コミュニティの責任者を移譲する必要あり
- ⇒ オンライン限定で sizuhiko さん名義に変更する
